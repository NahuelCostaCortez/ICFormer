...
import h5py
from sklearn.metrics import classification_report, mean_squared_error as rmse, accuracy_score
import sys
import utils
from models.ICFormer import ICFormer
import wandb
...
if __name__ == "__main__":
    ...
    y_regression_train = y_regression_train.reshape(y_regression_train.shape[0], y_regression_train.shape[1]*y_regression_train.shape[2])
    ...
    x_test = np.reshape(test_datasets_ICs, (test_datasets_ICs.shape[0]*test_datasets_ICs.shape[1], test_datasets_ICs.shape[2], test_datasets_ICs.shape[3]))
    y_regression_test = np.reshape(test_datasets_degradation_modes, (test_datasets_degradation_modes.shape[0], test_datasets_degradation_modes.shape[1], test_datasets_degradation_modes.shape[2]*test_datasets_degradation_modes.shape[3]))
    y_regression_test = np.reshape(y_regression_test, (y_regression_test.shape[0]*y_regression_test.shape[1], y_regression_test.shape[2]))
    y_classification_test = np.reshape(test_datasets_silent_modes, (test_datasets_silent_modes.shape[0]*test_datasets_silent_modes.shape[1],))
    ...
    callbacks = [tf.keras.callbacks.EarlyStopping(monitor='val_loss', mode='min', patience=10, restore_best_weights=True),
            tf.keras.callbacks.ModelCheckpoint(filepath='./checkpoints/ICFormer_best10_'+str(run.id)+'.hdf5', 
    ...
    history = model.fit(x_train, {'y_regression': y_regression_train, 'y_classification': y_classification_train},
            validation_data=(x_test, {'y_regression': y_regression_test, 'y_classification': y_classification_test}),
            epochs=1000, batch_size=128, callbacks=callbacks, verbose=2)
